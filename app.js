...
(function () {
  ...
  const state = {
    ...
    remoteSessionStarting: null,
    remoteSessionStartingId: '',
  ...
  async function init() {
    ...
    if (isRemoteMode) {
      // Render the phone remote immediately. Firebase connects in the background
      // so the phone does not sit on a blank/loading screen on slow networks.
      renderRemoteApp(firebaseInitPromise);
      return;
    }
  ...
  async function initRemoteFirestoreOnly() {
    // Remote QR must not hang on Firebase Auth. The session room is temporary
    // and should use public read/write rules on presentationHubSessions only.
    ...
    try {
      ...
      console.warn('Remote Firestore setup failed:', error);
      state.firebaseReady = false;
      updateFirebaseStatus('Remote offline');
      return false;
    }
  ...
  async function createFreshRemoteSession(preferredSessionId) {
    ...
    // First write a tiny session document. This is faster and gives the phone
    ...
    // Publish heavier state after the live session exists. Do not block QR.
    setTimeout(() => publishSessionState(true), 30);
    return state.sessionId;
  }

  function startFreshRemoteSessionInBackground(sessionId) {
    if (state.remoteSessionStarting && state.remoteSessionStartingId === sessionId) return state.remoteSessionStarting;
    state.remoteSessionStartingId = sessionId;
    state.remoteSessionStarting = createFreshRemoteSession(sessionId)
      .then((readyId) => {
        const badge = $('qrSessionBadge');
        if (badge && readyId === state.sessionId) badge.textContent = 'Ready';
        if (els.qrHelp && readyId === state.sessionId) {
          els.qrHelp.textContent = 'Ready. Scan Control QR for phone buttons.';
        }
        return readyId;
      })
      .catch((error) => {
    ...
    return state.remoteSessionStarting;
  }
  ...
  async function openQrModal() {
    ...
    // Fast path: create the phone links and QR immediately. The live Firebase
    // session starts in the background using the exact same session id.
    ...
    if (els.qrHelp) els.qrHelp.textContent = 'QR is ready. Starting live desktop session now...';
    ...
    startFreshRemoteSessionInBackground(sessionId).catch(() => {});
  }
  ...
  function renderRemoteApp(firebaseInitPromise) {
    document.documentElement.classList.add('presentation-hub-remote-mode');
    document.body.classList.add('presentation-hub-remote-mode');
    document.body.classList.remove('remote-fullscreen-open');
  ...
  function sendRemoteCommand(ref, action, value) {
    ...
    const command = { id, action, value: value ?? null, issuedAt: Date.now(), v: 13 };
    const mirror = action === 'magicEffect' || action === 'testMagicEffect'
      ? { lastMagicCommand: command, lastMagicCommandAt: Date.now() }
      : {};
    const write = ref.set({ command, ...mirror, commandUpdatedAt: Date.now() }, { merge: true });
    if (write && typeof write.catch === 'function') {
      write.catch((error) => {
        const pill = $('remoteStatusPill');
        const label = $('remoteFileLabel');
  ...
  function renderClassroomTools() {
    ...
    if (els.pickNameModeSelect) els.pickNameModeSelect.value = state.classroom.removePicked ? 'no-repeat' : 'allow-repeat';
    if (els.removePickedInput) els.removePickedInput.checked = state.classroom.removePicked;
    state.classroom.groupRollMode = ['balanced', 'no-repeat', 'free'].includes(state.classroom.groupRollMode)
      ? state.classroom.groupRollMode
      : (state.classroom.balanced === false ? 'free' : 'balanced');
    state.classroom.balanced = state.classroom.groupRollMode === 'balanced';
